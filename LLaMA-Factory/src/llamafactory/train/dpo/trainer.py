...
class CustomDPOTrainer(DPOTrainer):
    ...
    def lcpo_loss(self, chosen_logps: "torch.Tensor", rejected_logps: "torch.Tensor") -> "torch.Tensor":
        chosen_beta = 1
        rejected_beta = 1

        log_odds = (chosen_beta * chosen_logps - rejected_beta * rejected_logps) - (
            torch.log1p(-torch.exp(chosen_logps)) -
            torch.log1p(-torch.exp(rejected_logps))
        )
        ...
        lcpo_loss = self.beta * odds_ratio_loss
    ...
    def simper_loss(self, chosen_logps: "torch.Tensor", rejected_logps: "torch.Tensor") -> "torch.Tensor":
        r"""Compute SimPER loss for batched log probabilities of the policy model."""
        chosen_rewards = torch.exp(chosen_logps)
        rejected_rewards = torch.exp(rejected_logps)
        simper_loss = -chosen_rewards + rejected_rewards
        return simper_loss
    ...
    def compute_preference_loss(
        ...
        if not self.finetuning_args.use_ref_model:
            if self.loss_type == "orpo":
                ...
                losses = self.simper_loss(
                    policy_chosen_logps, policy_rejected_logps)
            elif self.loss_type == "lcpo":
                losses = self.lcpo_loss(
                    policy_chosen_logps, policy_rejected_logps)
            else:
    ...
    def concatenated_forward(
        ...
        r"""Compute the sum log probabilities of the labels under given logits if loss_type is not IPO, ORPO, LCPO, NA_LCPO or SimPO.
        ...
        all_logits: torch.Tensor = model(
            **batch, return_dict=True, use_cache=False).logits.to(torch.float32)
        all_logps, valid_length = get_batch_logps(logits=all_logits, labels=batch["labels"], ld_alpha=(self.ld_alpha if not is_ref_model else None))
        if self.loss_type in ["ipo", "orpo", "simpo", "simper", "lcpo", "na_lcpo"]:
        ...
        if self.loss_type in ["ipo", "orpo", "simpo", "simper", "lcpo"]:
            return chosen_logps, rejected_logps, chosen_logits, rejected_logits, chosen_logps, chosen_length, rejected_length
        else:
    ...
    def get_batch_loss_metrics(
        ...
        metrics[f"{prefix}length/gap"] = (rejected_length.float() -
                                          chosen_length.float()).mean().item()

        # if self.finetuning_args.use_ref_model:
        #     metrics[f"{prefix}/linear/ref_linear"] = ((reference_chosen_logps - reference_rejected_logps) / (
        #         rejected_length - chosen_length)).mean().item()
        # metrics[f"{prefix}/linear/policy_linear"] = (
        #     (policy_chosen_logps - policy_rejected_logps) / (rejected_length - chosen_length)).mean().item()
        # if self.finetuning_args.use_ref_model:
        #     metrics[f"{prefix}/linear/dpo_linear"] = ((policy_chosen_logps - policy_rejected_logps -
        #                                               reference_chosen_logps + reference_rejected_logps) / (rejected_length - chosen_length)).mean().item()
    ...
    def log(self, logs: dict[str, float], *args, **kwargs) -> None:
        ...
        for key, metrics in self._stored_metrics[train_eval].items():
            key_list.append(key)
            metric_list.append(torch.tensor(metrics, dtype=torch.float).to(
                self.accelerator.device).mean().item())
