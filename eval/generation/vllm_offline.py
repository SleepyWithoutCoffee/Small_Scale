...
def collect_and_merge_results(result_queue: Queue, dp_size: int, eval_data: pd.DataFrame) -> pd.DataFrame:
    """
    ...
    for _ in range(dp_size):
        try:
            result = result_queue.get(timeout=None)  # 5 minute timeout
            rank = result['rank']
    ...
    for rank in successful_ranks:
        ...
        # Assign the results to the correct rows
        for i, result in enumerate(all_results[rank]):
            if start_idx + i < len(merged_data):
                merged_data.iloc[start_idx + i,
                                    merged_data.columns.get_loc('model_output')] = result
                cnt += 1
...
def run(args):
    ...
    final_results = collect_and_merge_results(result_queue, dp_size, eval_data)
