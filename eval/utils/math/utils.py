...
def mathd_normalize_answer(answer: Optional[str]) -> Optional[str]:
    ...
    try:
        ...
        # print(f"Failed to normalize answer: {answer}")
        # print(f"Error: {e}")
        pass
    return answer
...
def _strip_string(string):
            ...
            for substr in substrs:
                ...
                if substr[0] == "{":
                    ...
                    try:
                        ...
                        # print(f"Failed to fix fracs: {string}")
                        # print(f"Error: {e}")
                        return string
                    a = substr[0]
                    b = substr[1]
    ...
    def _fix_a_slash_b(string):
        ...
        try:
            ...
            # print(f"Failed to fix a/b: {string}")
            # print(f"Error: {e}")
            return string
...
def _is_int(x: float) -> bool:
    ...
    try:
        ...
        # print(f"Failed to check if {x} is an int")
        # print(f"Error: {e}")
        return False
...
def _normalize(expr: str) -> str:
    ...
    if "\\" in expr:
        try:
            ...
            # print(f"Failed to parse latex: {expr}")
            # print(f"Error: {e}")
            pass
...
def remove_boxed(s):
    ...
    try:
        ...
        # print(f"Failed to remove \\boxed command from {s}")
        # print(f"Error: {e}")
        return None
            ...
            for substr in substrs:
                if substr[0] == "{":
                    try:
                        ...
                        return string
                    a = substr[1:substr.find(
                        '}')] if '}' in substr else substr[0]
                    if '\\' not in a:
                        return a
        return string

    def _change_frac_0_order(string):
        if '+' in string:
            substrs = string.split('+')
            if len(substrs) == 2:
                return substrs[1] + '+' + substrs[0]
        return string

    # be at least as lenient as mathd
    if ground_truth_normalized_mathd == given_answer_normalized_mathd:
        return True
    if given_answer_normalized_mathd is not None and given_answer_normalized_mathd.count('\\frac') == 1:
        frac_0 = _get_frac_0(given_answer_normalized_mathd)
        changed_frac_0 = _change_frac_0_order(frac_0)
        if ground_truth_normalized_mathd == given_answer_normalized_mathd.replace(frac_0, changed_frac_0):
            return True
    return False

def patch_th_grade(answer: str) -> str:
    r"""Patch a corner case in MATH-500."""
    if answer is not None and r"\text{th grade}" in answer:
        return answer.replace(r"\text{th grade}", "")
    return answer


def patch_pm(answer: str) -> str:
    r"""Patch a corner case in MATH-500."""
    if answer is not None and "2:00" in answer:
        return answer.replace("2:00", "2")
    return answer